import { Client, ColorResolvable, Guild, User, GuildEmoji } from 'discord.js'
...
export default class WOKCommands extends EventEmitter {
  ...
  private async setUp(client: Client, options?: Options) {
    ...
    if (mongoUri) {
      ...
      const results: any[] = await guildConfig.find({})

      for (const result of results) {
        const { _id } = result
        this._config[_id] = result
      }
      const userResults: any[] = await guildConfig.find({})

      for (const userResult of userResults) {
        const { _id } = userResult
        this.setUserData(_id, userResult)
      }
    } else {
      if (showWarns) {
        console.warn(
  ...
  public getUserData(user: User | null): Map<string,any> {
    return this._userdata.get(user ? user.id : '') || null
  }
  public get userdata(): any {
    return this._userdata
  }
  public setUserData(user: User | null, data: any): WOKCommands {
    if (user) {
      this._userdata.set(user.id, data)
    } 
    return this
  }
  public getConfig(guild: Guild | null): any {
    return this._config[guild ? guild.id : ''] || null
  }
  public setConfig(guild: Guild | null, config: any): WOKCommands {
    if (guild) {
      this._config[guild.id] = config
    }
    return this
  }
  public getPrefix(guild: Guild | null): string {
    return this._config[guild ? guild.id : ''].prefix || this._defaultPrefix
  }

  public setPrefix(guild: Guild | null, prefix: string): WOKCommands {
    if (guild) {
      this._config[guild.id] ? this._config[guild.id].prefix = prefix : this._config[guild.id] = {prefix,language:this._defaultLanguage}
    }
    return this
  }
