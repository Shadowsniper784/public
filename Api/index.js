...
const configSchema = require('../Bot/dist/models/config')
...
router.post('/guilds',async (req, res) => {
  ...
  console.log(req.body)
    if(!req.body.access_token) return res.status(400).json({error:'no access token'})
...
router.post('/refresh', async (req, res) => {
  try {
  if(!req.body.refresh_token) return res.status(400).json({error:'no refresh'})
    console.log(req.body)
